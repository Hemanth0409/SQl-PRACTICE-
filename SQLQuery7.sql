create table  Salesman (Salseman_id int primary key,name varchar(25) unique not null ,city varchar(25) not null,commission numeric(4,2) not null )
...
create table Customer ( customer_id int primary key ,cust_name varchar(25),city varchar(25), grade int,salesman_id int references salesman(salseman_id))
...
create  table Orders (ord_no int,purch_amt numeric(8,3), ord_date date,customer_id int references customer(customer_id),salesman_id int references salesman(salseman_id))
...
select sm.name as SalesMan,c.cust_name,c.city from Salesman  as sm inner join Customer as c on sm.city=c.city
...
select  c.Cust_name as Customer_name,sm.city,sm.name as Salesman,sm.commission from Salesman as sm inner join customer as c on sm.salseman_id=c.salesman_id  where sm.commission > 0.12;
...
select  c.Cust_name as Customer_name,c.city as customer_city,sm.name as Salesman,sm.city as salesman_city from Salesman as sm inner join customer as c on sm.salseman_id=c.salesman_id  order by c.customer_id asc;
